...
public class CheckNet {
    ...
    private void request(final CheckNetResponse checkNetResponse) {
        ...
        if (activeNetwork != null && activeNetwork.isConnected()) {
            ...
            checkNetResponse.sendFinishMessage(0, "无法连接到博山庐,请打开网络连接");
        }
    }

    private void checkSchoolNet(Context context) {
        HttpUtil.get(App.LOGIN, new ResponseHandler() {
            @Override
            ...
            public void onFinish() {
                checkLoginResult(context, pcResponse);
            }
        });
    }


    private void checkLoginResult(Context context, String response) {
        if (TextUtils.isEmpty(response)) {
            errCount++;
            if (errCount == 2) {
                Toast.makeText(context, "无法连接到博山庐,请检查你的网络连接！",
